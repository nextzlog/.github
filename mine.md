...
$$
\forall\boldsymbol{x},\boldsymbol{y}\colon \left(\boldsymbol{x},\boldsymbol{y}\right)\in\mathbb{T} \Rightarrow \boldsymbol{y} \approx f(\boldsymbol{x}).
 \qquad(1.1)$$
...
\boldsymbol{x} = \boldsymbol{y} \Leftrightarrow d(\boldsymbol{x},\boldsymbol{y}) = 0.
 \qquad(1.2)$$

最近傍法は、他の著名な教師あり学習の手法と比較して、事前の学習が不要である点が特徴的で、**遅延学習**と呼ばれる。
以上の議論に基づき、最近傍法を実装しよう。引数は、参照する近傍点の個数$$K$$と、集合$$\left\{\boldsymbol{x},y\right\}$$と、距離関数$$d$$である。
...
\boldsymbol{y} + \varepsilon \approx f(\boldsymbol{x}) = \displaystyle\sum_{k=0}^K w_k \phi_k(\boldsymbol{x}) = {}^t\boldsymbol{w}\boldsymbol{\phi}(\boldsymbol{x}).
 \qquad(1.3)$$
...
\mathrm{arg\,min}_f \left\{y-f(\boldsymbol{x})\right\}^2 =
\mathrm{arg\,min}_f E(\boldsymbol{w}).
 \qquad(1.5)$$
...
\hat{\boldsymbol{w}} = \boldsymbol{w} - \eta \nabla E(\boldsymbol{w}) = \boldsymbol{w} + \eta \displaystyle\sum_{n=1}^N \{y_n - {}^t\boldsymbol{w} \boldsymbol{\phi}(\boldsymbol{x}_n)\} \boldsymbol{\phi}(\boldsymbol{x}_n),
\enspace\mathrm{where}\enspace
\eta \ll \left|\displaystyle\frac{\boldsymbol{w}}{\nabla E(\boldsymbol{w})}\right|.
 \qquad(1.6)$$

定数$$\eta$$を**学習率**と呼ぶ。以上の議論に基づき、線型回帰を実装する。引数は、学習率$$\eta$$と、集合$$\left\{x,y\right\}$$と、基底$$\Phi$$である。
...
f_\mathrm{sigm}(z) = \displaystyle\frac{1}{1 + e^{-z}} = \displaystyle\frac{1}{2}\tanh\displaystyle\frac{z}{2} + \displaystyle\frac{1}{2}.
 \qquad(2.3)$$
...
\enspace\mathrm{where}\enspace
E_\mathrm{sq}(\hat{\boldsymbol{y}},\boldsymbol{y}) = \displaystyle\frac{1}{2} \left\|\boldsymbol{y}-\hat{\boldsymbol{y}}\right\|^2.
 \qquad(2.7)$$
...
= - y^k + \displaystyle\sum_{i=1}^K y^i \hat{y}^k = -y^k + \hat{y}^k.
 \qquad(2.11)$$
...
\boldsymbol{y}^t = f(\boldsymbol{z}^t) = f(W_i\boldsymbol{x}^t+W_h\boldsymbol{y}^{t-1}).
 \qquad(2.15)$$
...
\hat{y} = \mathrm{sign}\left(\boldsymbol{w} \cdot \boldsymbol{x}+c\right) \in \left\{1,-1\right\}.
 \qquad(3.1)$$
...
f(\boldsymbol{w}) - \displaystyle\sum_{i=1}^N \lambda_n \{y_n(\boldsymbol{w} \cdot \boldsymbol{x}_n + c) - 1 + \xi_n\} - \displaystyle\sum_{i=1}^N \mu_n \xi_n.
 \qquad(3.6)$$
...
\lambda_n \left\{y_n\left(\boldsymbol{w} \cdot \boldsymbol{x}_n + c\right) - 1 + \xi_n\right\} = 0,\;
\left\{
\begin{aligned}
\lambda_n &\geq 0, \\
\mu_n     &\geq 0, \\
...
\displaystyle\sum_{i=1}^N \lambda_i \left\{1 - \displaystyle\frac{1}{2} \displaystyle\sum_{j=1}^N \lambda_j y_i y_j (\boldsymbol{x}_i \cdot \boldsymbol{x}_j)\right\} \leq f(\boldsymbol{w}).
 \qquad(3.9)$$
...
\end{aligned}
\right\}
\Leftarrow 0 = \displaystyle\sum_{i=1}^N \lambda_i y_i.
 \qquad(3.10)$$
...
\displaystyle\frac{\partial \tilde{L}}{\partial \delta_i} = y_i (y_i-y_j)-\delta_i \left|\boldsymbol{x}_i-\boldsymbol{x}_j\right|^2-y_i \displaystyle\sum_{n=1}^N \lambda_n y_n \boldsymbol{x}_n \cdot (\boldsymbol{x}_i-\boldsymbol{x}_j).
 \qquad(3.11)$$
...
\delta_i = -\displaystyle\frac{y_i}{\left|\boldsymbol{x}_i-\boldsymbol{x}_j\right|^2} \left\{\displaystyle\sum_{n=1}^N \lambda_n y_n \boldsymbol{x}_n \cdot (\boldsymbol{x}_i-\boldsymbol{x}_j)-y_i+y_j\right\}.
 \qquad(3.12)$$
...
c = -\displaystyle\frac{1}{2} \left\{
\min_{i|y_i=+1} \displaystyle\sum_{j=1}^N \lambda_j y_j \boldsymbol{x}_i \cdot \boldsymbol{x}_j +
\max_{j|y_j=-1} \displaystyle\sum_{i=1}^N \lambda_i y_i \boldsymbol{x}_j \cdot \boldsymbol{x}_j\right\}.
 \qquad(3.13)$$
...
\Phi: \boldsymbol{x} \mapsto \Phi_{\boldsymbol{x}}.
 \qquad(3.14)$$
...
\exp \left\{- \displaystyle\frac{1}{2\sigma^2} \displaystyle\sum_{d=1}^D \left\|\boldsymbol{x}_i-\boldsymbol{x}_j\right\|^2\right\}
\Leftarrow
...
\displaystyle\sum_{i=1}^N \displaystyle\sum_{j=1}^N a_i b_j k(\boldsymbol{x}_i, \boldsymbol{x}_j).
 \qquad(3.19)$$
...
\left\langle \boldsymbol{x},\boldsymbol{x}\right\rangle \geq 0.
 \qquad(3.20)$$
...
\left\langle f,g\right\rangle = \int_H f(\boldsymbol{x}) \overline{g(\boldsymbol{x})} d\mu(\boldsymbol{x}).
 \qquad(3.21)$$
...
\left\|\boldsymbol{x}\right\| = \left\langle \boldsymbol{x},\boldsymbol{x}\right\rangle^{\frac{1}{2}} \in \mathbb{R}.
 \qquad(3.22)$$
...
d(\boldsymbol{x},\boldsymbol{y}) = \left\|\boldsymbol{x}-\boldsymbol{y}\right\| \geq 0.
 \qquad(3.23)$$
...
H_0 = \mathrm{span}\left\{\displaystyle\sum_{n=1}^n a_n k(\boldsymbol{x}_n,\cdot)|a_n \in \mathbb{R}\right\}.
 \qquad(3.24)$$
...
f(\boldsymbol{x}) = \displaystyle\sum_{n=1}^N a_n k(\boldsymbol{x}_n,\boldsymbol{x}) = \left\langle f,k(\cdot,\boldsymbol{x})\right\rangle_{H_0}.
 \qquad(3.26)$$
...
k(x,y) = \displaystyle\frac{a}{\pi} \mathrm{sinc} a(y-x).
 \qquad(3.28)$$
...
k(\boldsymbol{w},\boldsymbol{x}) = \tanh {}^t\boldsymbol{w} \boldsymbol{x}.
 \qquad(3.30)$$
...
決定木の学習では、意思決定の事例の集合$$\left\{\boldsymbol{x},y\right\}$$に対し、簡潔で解釈の容易な質問と条件分岐と、その順序を習得する。
...
$$
C(\boldsymbol{x}) \colon \boldsymbol{x} \mapsto \boldsymbol{s} \in \left\{0, 10, 11\right\}.
 \qquad(4.2)$$
...
p\left(\boldsymbol{x}_i\right) \leq p\left(\boldsymbol{x}_j\right) \Leftrightarrow I(\boldsymbol{x}_i) \geq I(\boldsymbol{x}_j).
 \qquad(4.3)$$
...
I(\boldsymbol{x}_1,\boldsymbol{x}_2,\ldots,\boldsymbol{x}_N) = \displaystyle\sum_{n=1}^N I(\boldsymbol{x}_n).
 \qquad(4.4)$$
...
I(\boldsymbol{x}) = \log_2 \displaystyle\frac{1}{p\left(\boldsymbol{x}\right)} = - \log_2 p\left(\boldsymbol{y}\right) \geq 0.
 \qquad(4.5)$$
...
\overline{L(C)} \geq H(p) = \displaystyle\sum_{\boldsymbol{x}} p\left(\boldsymbol{x}\right) I(\boldsymbol{x}) = -\displaystyle\sum_{\boldsymbol{x}} p\left(\boldsymbol{x}\right) \log_2 p\left(\boldsymbol{x}\right) \geq 0.
 \qquad(4.6)$$
...
G(Q) = H(X) - H(X|Q) = H(X) - \displaystyle\sum_{k=0}^{K-1} P\left(X_k\,\middle|\,X\right) H(X_k) \geq 0.
 \qquad(4.7)$$
...
次に、決定木の本体を実装する。引数は、決定木が学習する集合$$\left\{\boldsymbol{x},y\right\}$$と、決定木の末端の細分化を抑える閾値である。
...
次に、条件分岐を実装する。引数は、決定木が学習する集合$$\left\{\boldsymbol{x},y\right\}$$と、条件$$\boldsymbol{x}$$を分割する軸と、分割を行う閾値である。
...
![images/id3.plain.svg](images/id3.plain.svg)

(1) $$\mathtt{limit}=1e-5$$.

![images/id3.prune.svg](images/id3.prune.svg)
...
\int_{\boldsymbol{x}}p\left(\boldsymbol{x}\right)(y-\hat{f}(\boldsymbol{x}))^2d\boldsymbol{x} = \mathbf{V}\!\left[\,y-f(\boldsymbol{x})\,\right] + \left(\underset{}{\mathbf{E}}\!\left[\,\hat{f}(\boldsymbol{x})\,\right] - f(\boldsymbol{x})\right)^2 + \mathbf{V}\!\left[\,\hat{f}(\boldsymbol{x})\,\right].
 \qquad(4.9)$$
...
\hat{f}(\boldsymbol{x}) = \displaystyle\frac{1}{T} \displaystyle\sum_{t=1}^T \hat{f}_t(\boldsymbol{x}).
 \qquad(4.10)$$
...
\mathbf{V}\!\left[\,\hat{f}(\boldsymbol{x})\,\right] = \displaystyle\frac{1}{T^2}\displaystyle\sum_{i=1}^T \displaystyle\sum_{j=1}^T \mathbf{C}\!\left[\,f_i(\boldsymbol{x}),f_j(\boldsymbol{x})\,\right].
 \qquad(4.11)$$
...
$$
\hat{f}(\boldsymbol{x}) = \displaystyle\sum_{t=1}^T w_t \hat{f}_t(\boldsymbol{x}).
 \qquad(4.12)$$
...
E(\hat{\boldsymbol{f}}) =
\exp \left\{-{}^t\boldsymbol{f}(\boldsymbol{x}) \hat{\boldsymbol{f}}(\boldsymbol{x})\right\} =
\exp \left\{-{}^t\boldsymbol{f}(\boldsymbol{x}) \displaystyle\sum_{t=1}^T w_t \hat{\boldsymbol{f}}_t(\boldsymbol{x})\right\} \geq 0.
 \qquad(4.13)$$
...
E(\hat{\boldsymbol{f}}) =
q_T(\boldsymbol{x}) \exp \left\{-\boldsymbol{f}(\boldsymbol{x}) w_T \hat{\boldsymbol{f}}_T(\boldsymbol{x})\right\},
\enspace\mathrm{where}\enspace
q_T(\boldsymbol{x}) =
\exp \left\{-\boldsymbol{f}(\boldsymbol{x}) \displaystyle\sum_{t=1}^{T-1} w_t \hat{\boldsymbol{f}}_t(\boldsymbol{x})\right\}.
 \qquad(4.16)$$
...
![images/id3.bag50.svg](images/id3.bag50.svg)

(1) Bagging  class.

![images/id3.ada50.svg](images/id3.ada50.svg)
...
\left(\displaystyle\prod_{v=1}^V \phi_{zv}^{N_v}\right) \mathrm{Dir}\left(\phi\,\middle|\,\nu\right) \left(\displaystyle\prod_{k=1}^K \theta_k^{N_k}\right) \mathrm{Dir}\left(\theta\,\middle|\,\alpha\right).
 \qquad(5.1)$$
...
\displaystyle\frac{1}{\mathrm{B}\left(N\right)} \displaystyle\prod_{k=1}^K \theta_k^{N_k-1}.
 \qquad(5.3)$$
...
\Gamma\left(n\right) = \int_0^{\infty} x^{n-1} e^{-x} dx.
 \qquad(5.4)$$
...
\displaystyle\frac{1}{\mathrm{B}\left(\hat{\alpha}\right)} \displaystyle\prod_{k=1}^K \theta_k^{\hat{\alpha}_k-1}.
 \qquad(5.7)$$
...
\displaystyle\frac{1}{\tilde{\mathcal{N}}\left(S\right)} \exp \left\{-\displaystyle\frac{1}{2} {}^t(\boldsymbol{x}-\boldsymbol{\mu}) S^{-1} (\boldsymbol{x}-\boldsymbol{\mu})\right\},
\enspace\mathrm{where}\enspace
\tilde{\mathcal{N}}\left(S\right) = \sqrt{(2\pi)^D\left|S\right|}.
 \qquad(6.2)$$
...
\displaystyle\frac{w_k \mathcal{N}\left(\boldsymbol{x}\,\middle|\,\boldsymbol{\mu}_k,S_k\right)}{p\left(\boldsymbol{x}\right)}.
 \qquad(6.4)$$
...
![images/gmm.truth.svg](images/gmm.truth.svg)

(1) cluster map.

![images/gmm.dense.svg](images/gmm.dense.svg)
...
\Leftarrow
\displaystyle\frac{\partial \mathcal{D}}{\partial \boldsymbol{\mu}_k} = 0.
 \qquad(6.7)$$
...
![images/gmm.km.k3.svg](images/gmm.km.k3.svg)
...
\displaystyle\prod_{n=1}^N \displaystyle\sum_{k=1}^K w_k \mathcal{N}\left(\boldsymbol{x}_n\,\middle|\,\boldsymbol{\mu}_k,S_k\right).
 \qquad(6.9)$$
...
\displaystyle\sum_{n=1}^N \gamma_{nk} S_k^{-1} (\boldsymbol{x}_n - \boldsymbol{\mu}_k).
 \qquad(6.10)$$
...
\right\}
\enspace\mathrm{where}\enspace
N_k = \displaystyle\sum_{n=1}^N \gamma_{nk}.
 \qquad(6.11)$$
...
\enspace\mathrm{where}\enspace
\displaystyle\sum_{n=1}^N \gamma_n=1.
 \qquad(6.12)$$
...
\lim_{\lambda\to0} \lambda\log \left\{w \mathcal{N}\left(\boldsymbol{x}\,\middle|\,\boldsymbol{\mu},\lambda E\right)\right\} =
\lim_{\lambda\to0} \left\{\lambda\log w - \lambda\displaystyle\frac{D}{2} \log (2\pi\lambda) - \displaystyle\frac{1}{2} \left\|\boldsymbol{x}-\boldsymbol{\mu}\right\|^2\right\} =
...
![images/gmm.em.k3.svg](images/gmm.em.k3.svg)
...
\int p\left(\boldsymbol{x}\,\middle|\,z\right) p\left(z\,\middle|\,\theta\right) dz.
 \qquad(6.18)$$
...
\iint p\left(\boldsymbol{x}\,\middle|\,z\right) p\left(z\,\middle|\,\theta\right) p\left(\theta\,\middle|\,\phi\right) dz d\theta.
 \qquad(6.19)$$
...
\iint \hat{p}\left(z,\theta\right) \log \displaystyle\frac{p\left(\boldsymbol{x},z,\theta\right)}{\hat{p}\left(z,\theta\right)} dz d\theta = F(\hat{p}).
 \qquad(6.20)$$
...
\displaystyle\frac{\partial }{\partial f} \int f(z) g(\theta) \log \displaystyle\frac{p\left(\boldsymbol{x},z,\theta\right)}{f(z)g(\theta)} d\theta = 0.
 \qquad(6.23)$$
...
\int g(\theta) \log \displaystyle\frac{p\left(\boldsymbol{x},z,\theta\right)}{f(z)g(\theta)} d\theta - 1 = 0.
 \qquad(6.24)$$
...
F(\hat{p}) \simeq
\underset{f,g}{\mathbf{E}}\!\left[\,\log \displaystyle\frac{p\left(\boldsymbol{x},z\,\middle|\,\theta\right)}{f(z)}\,\right] - \displaystyle\frac{\hat{\left|\theta\right|}}{2} \log N + \log p\left(\hat{\theta}\right).
 \qquad(6.28)$$
...
p\left(\boldsymbol{\mu}\,\middle|\,S\right) = \displaystyle\prod_{k=1}^K \mathcal{N}\left(\boldsymbol{\mu}_k\,\middle|\,\boldsymbol{m}_k,\sigma_k^{-1} S_k\right).
 \qquad(6.31)$$
...
![images/gmm.vb.k3.svg](images/gmm.vb.k3.svg)
...
p\left(\boldsymbol{x}\,\middle|\,z,\boldsymbol{\mu},S\right) p\left(z\,\middle|\,w\right) p\left(w) p\left(\boldsymbol{\mu}\,\middle|\,S\right) p(S\right).
 \qquad(6.34)$$
...
\exp \displaystyle\sum_{n=1}^N \displaystyle\sum_{k=1}^K z_{nk} \left\{\underset{w}{\mathbf{E}}\!\left[\,\log w_k\,\right] + \underset{\boldsymbol{\mu},S}{\mathbf{E}}\!\left[\,\log \mathcal{N}\left(\boldsymbol{x}_n\,\middle|\,\boldsymbol{\mu}_k,S_k\right)\,\right]\right\} \propto
\displaystyle\prod_{n=1}^N \displaystyle\prod_{k=1}^K \gamma_{nk}^{z_{nk}}.
 \qquad(6.35)$$
...
\psi\left(\alpha_k\right) - \psi\left(\displaystyle\sum_{j=1}^K \alpha_j\right).
 \qquad(6.36)$$
...
- \displaystyle\frac{1}{2} \underset{\boldsymbol{\mu},S}{\mathbf{E}}\!\left[\,D \log 2\pi + \log \left|S_k\right| + {}^t(\boldsymbol{x}_n - \boldsymbol{\mu}_k) S_k^{-1} (\boldsymbol{x}_n - \boldsymbol{\mu}_k)\,\right].
 \qquad(6.37)$$
...
\nu_k {}^t(\boldsymbol{x}_n - \boldsymbol{m}_k) W_k (\boldsymbol{x}_n - \boldsymbol{m}_k) + \displaystyle\frac{D}{\sigma_k}.
 \qquad(6.39)$$
...
g(\theta) \propto \exp\left\{\underset{z}{\mathbf{E}}\!\left[\,\log p\left(\boldsymbol{x},z\,\middle|\,w,\boldsymbol{\mu},S\right)\,\right] + \log p\left(w) + \log p\left(\boldsymbol{\mu}\,\middle|\,S\right) + \log p(S\right)\right\}.
 \qquad(6.40)$$
...
\left\{D \log 2\pi + \left|S_k\right| + {}^t(\boldsymbol{x}_n-\hat{\boldsymbol{\mu}}_k) S_k^{-1} (\boldsymbol{x}_n-\hat{\boldsymbol{\mu}}_k) - 2 \log w_k\right\}.
 \qquad(6.41)$$
...
\underset{z}{\mathbf{E}}\!\left[\,\log p\left(\boldsymbol{x},z\,\middle|\,\theta\right)\,\right] + \log p\left(\theta\,\middle|\,\phi\right) - \underset{z}{\mathbf{E}}\!\left[\,\log p\left(\boldsymbol{x},z\right)\,\right].
 \qquad(6.42)$$
...
\hat{\boldsymbol{m}}_k = \displaystyle\frac{1}{\hat{\sigma}_k} \left(\sigma_k \boldsymbol{m}_k + \displaystyle\sum_{n=1}^N \gamma_{nk} \boldsymbol{x}_n\right).
 \qquad(6.44)$$
...
\hat{W}_k^{-1} =
W_k^{-1}+\displaystyle\sum_{n=1}^N\gamma_{nk}\boldsymbol{x}_n{}^t\boldsymbol{x}_n+\sigma_k\boldsymbol{m}_k{}^t\boldsymbol{m}_k-\hat{\sigma}_k\hat{\boldsymbol{m}}_k{}^t\hat{\boldsymbol{m}}_k.
 \qquad(6.45)$$
...
\mathrm{tr}\left((\boldsymbol{x} {}^t\boldsymbol{x}) W\right).
 \qquad(6.46)$$
